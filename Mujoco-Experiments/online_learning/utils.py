...
def load_expert(num_inputs, action_space, args, device):
    if args.expert_type == 'sac':
        expert = SACPolicy(num_inputs, action_space.shape[0], 256, action_space).to(device)
        expert.load_state_dict(torch.load(args.expert_params_path,map_location=device))
    elif args.expert_type == 'ppo':
        expert = PPOPolicy(args.env_name).to(device)
    else:
        raise Exception()
    return expert
...
def avg_dicts(dict_list):
    ...
    for key in dict_list[0].keys():
        ...
        if torch.is_tensor(avg_dict[key]):
            avg_dict[key] = avg_dict[key].cpu().detach().numpy()
    return avg_dict
def timer(start,end):
