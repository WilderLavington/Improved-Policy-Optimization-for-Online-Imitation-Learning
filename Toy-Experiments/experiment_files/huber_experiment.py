...
def initialize(info):
    ...
    if info.problem_type == 'norm':
        ...
        raise Exception('')
    # set info
    info.get_loss_grad = Huber_grad
    info.get_loss = Huber_loss
...
def main():
    ...
    #
    if info.exp_sweep:
        run_batch_experiments(info, name='huber_exp_'+info.exp_id)
        plot_experiments(name='huber_exp_'+info.exp_id)
    else:
