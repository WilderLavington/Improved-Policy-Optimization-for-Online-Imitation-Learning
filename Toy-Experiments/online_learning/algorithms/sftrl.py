...
def SFTRL_step(info, k):
    w_k = info.w
    prev_X_grad = SFTRL_grad(info, info.X, info.b, info.w, w_k, k, info.param)
    for iter_ in range(1,info.max_inner_steps):
        w_grad = SFTRL_grad(info, info.X, info.b, info.w, w_k, k, info.param) / (k+1)
        get_loss_ = lambda w_: SFTRL_loss(info, info.X, info.b, w_, w_k, k, info.param) / (k+1)
...
#         lr = 2 * ac_lr
    ...
    # info added
    info.policy_loss_grad_norm_list.append(info.get_loss_grad(info.w, info.X, info.b).norm(2).pow(2))
    info.eta_list.append(np.log(k))
    info.inner_loop_steps_list.append(iter_)
    info.algo_loss_grad_norm_list.append(w_grad.abs().sum())
    return info
def SFTRL_grad(info, X, b, w, w_k, k=None,param=None):
    w_grad = info.get_loss_grad(w, X, b)
    if k > 1:
        w_grad -= info.get_loss_grad(w_k, X[info.SAMPLE_SIZE:,:], b[info.SAMPLE_SIZE:])
    w_grad += np.log(k) * (param) * (w - w_k)
    return w_grad
def SFTRL_loss(info, X, b, w, w_k, k=None,param=None):
    ...
    if k > 1:
        ...
        lin_loss = 0.
    tr_loss = 0.5 * np.log(k) * (param) * (w - w_k).pow(2).sum()
